...
namespace RobusAPI.Integrations.CoursesAndStudentsApiV2
{
    ...
    {
        private const string baseUrl = "https://localhost:7199/apiv2/";
        ...
        private HttpClient CreateHttpClient()
        {
            ...
            var authorizationBytes = Encoding.ASCII.GetBytes(authorizationText);
            var authotrizationHeaderValue = Convert.ToBase64String(authorizationBytes);
            var authorizationHeader = new AuthenticationHeaderValue("Basic", authotrizationHeaderValue);
                ...
                using (var httpClient = CreateHttpClient())
                {
                    ...
                    using (var responseMessage = await httpClient.GetAsync(url, cancellationToken ?? CancellationToken.None))
                    {
                        if (responseMessage.StatusCode == HttpStatusCode.OK)
                        {
                            ...
                            var courses = JsonSerializer.Deserialize<Course[]>(responseBody, new JsonSerializerOptions(JsonSerializerDefaults.Web));
        ...
        public async Task<Course> GetCourse(int courseId, CancellationToken? cancellationToken = null)
        {
                ...
                using (var httpClient = CreateHttpClient())
                {
                    ...
                    using (var responseMessage = await httpClient.GetAsync(url, cancellationToken ?? CancellationToken.None))
                    {
                        if(responseMessage.StatusCode == HttpStatusCode.OK)
                        {
                            ...
                            var course = JsonSerializer.Deserialize<Course>(responseBody, new JsonSerializerOptions(JsonSerializerDefaults.Web));
                            return course;
                        }
            ...
            {
                throw new ApiException("Unexpected exception", ex);
            }
        }
            ...
            try
            {
                using (var httpClient = CreateHttpClient())
                {
                    ...
                    using (var httpContent = new StringContent(requestJson))
                    {
                        ...
                        using (var responseMessage = await httpClient.PostAsync("courses", httpContent, cancellationToken ?? CancellationToken.None))
                        {
                            ...
                            {
                                var responseBody = await responseMessage.Content.ReadAsStringAsync(cancellationToken ?? CancellationToken.None);
                                var response = JsonSerializer.Deserialize<AddCourse422Response>(responseBody, new JsonSerializerOptions(JsonSerializerDefaults.Web));
                ...
                using (var httpClient = CreateHttpClient())
                {
                    ...
                    using (var httpContent = new StringContent(requestJson))
                    {
                        ...
                        using (var responseMessage = await httpClient.PutAsync(url, httpContent, cancellationToken ?? CancellationToken.None))
                        {
                            ...
                            if (responseMessage.StatusCode == HttpStatusCode.NotFound)
                                throw new CourseOrStudentNotFoundException("Bad credentials found");
            ...
            try
            {
                using (var httpClient = CreateHttpClient())
                {
                    ...
                    using (var responseMessage = await httpClient.DeleteAsync(url, cancellationToken ?? CancellationToken.None))
                    {
                        ...
                        if (responseMessage.StatusCode == HttpStatusCode.NotFound)
                            throw new CourseOrStudentNotFoundException("Not found");
            ...
            CancellationToken? cancellationToken = null)
        {
            try
            {
                using (var httpClient = CreateHttpClient())
                {
                    using (var httpContent = new MultipartFormDataContent("Upload----"+DateTime.Now.ToString(CultureInfo.InvariantCulture)))
                    {
            ...
            try
            {
                using (var httpClient = CreateHttpClient())
                {
                    ...
                    using (var responseMessage = await httpClient.DeleteAsync(url, cancellationToken ?? CancellationToken.None))
                    {
                        ...
                        if (responseMessage.StatusCode == HttpStatusCode.NotFound)
                            throw new CourseOrStudentNotFoundException($"Student {studentId} or course {courseId} not found ");
